...
namespace ARSoft.Tools.Net
{
	...
	public class DomainName : IEquatable<DomainName>, IComparable<DomainName>
	{
		...
		public static DomainName Root { 
            get{
                return new DomainName(new string[] { });
            }
        }

		internal static DomainName Asterisk { 
            get { 
		...
		{
			if (removeLabels < 0)
                throw new ArgumentOutOfRangeException(PropertyGet.GetPropertyName(() => removeLabels));
		...
		public static DomainName Parse(string s)
		{
			...
			throw new ArgumentException("Domain name could not be parsed", PropertyGet.GetPropertyName(() => s));
		}
		...
		private static readonly Regex _asciiNameRegex = new Regex("^[a-zA-Z0-9_-]+$", RegexOptions.CultureInvariant);
		...
		public override string ToString()
		{
			...
			return (_toString = String.Join(".", _labels.Select(x => x.ToMasterfileLabelRepresentation(true)).ToArray()) + ".");
		}
